...
@section('title', 'Adaugă intrare cunoștințe')
@section('page-title', 'Adaugă intrare cunoștințe')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('admin.chatbot.knowledge.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="question_example" value="{{ old('question_example') }}"
                       placeholder="Ex: Cum mă înscriu ca meseriaș?"
                ...
                <p class="text-xs text-gray-400 mt-1">Doar pentru referință — nu e afișat utilizatorilor</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Keywords (trigger) <span class="text-red-500">*</span></label>
                <input type="text" name="keywords" value="{{ old('keywords') }}"
                       placeholder="inscrie,meserias,devino meserias,vreau cont,cont nou"
                ...
                <p class="text-xs text-gray-400 mt-1">Separate prin virgulă. Dacă mesajul conține oricare din ele, se folosește acest răspuns.</p>
            </div>
            ...
            <div>
                ...
                <textarea name="answer" rows="4"
                          placeholder="Ex: Te poți înscrie gratuit ca meseriaș accesând linkul de mai jos. Durează 2 minute și contul tău va fi activ imediat."
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 @error('answer') border-red-400 @enderror">{{ old('answer') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Lasă gol pentru a nu afișa buton</p>
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="priority" value="{{ old('priority', 0) }}" min="0" max="100"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    <p class="text-xs text-gray-400 mt-1">0-100. Mai mare = verificat primul.</p>
                </div>
                <div class="flex items-end pb-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" checked class="w-4 h-4 text-red-600 rounded">
