                ...
                <tr class="{{ $craftsman->is_featured ? 'bg-yellow-50' : '' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center gap-2">
                            ...
                            <!-- Delete -->
                            <form action="{{ route('admin.craftsmen.destroy', $craftsman->id) }}" method="POST" class="inline"
                                  onsubmit="return confirm('Ești sigur că vrei să ștergi definitiv contul lui {{ $craftsman->name }}? Această acțiune nu poate fi anulată.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-800" title="Șterge definitiv">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
