...
<a href="{{ route('admin.leads.index') }}" class="flex items-center px-6 py-3 text-gray-300 hover:bg-gray-800 hover:text-white transition {{ request()->routeIs('admin.leads*') ? 'bg-gray-800 text-white border-l-4 border-primary-600' : '' }}">
    @php $newLeads = \App\Models\CraftsmanLead::where('status', 'nou')->count(); @endphp
    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/>
    </svg>
    Recrutare Meseriași
    @if($newLeads > 0)
        <span class="ml-auto bg-orange-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $newLeads }}</span>
    @endif
</a>
...
<a href="{{ route('admin.chatbot.index') }}" class="flex items-center px-6 py-3 text-gray-300 hover:bg-gray-800 hover:text-white transition {{ request()->routeIs('admin.chatbot.index') || request()->routeIs('admin.chatbot.show') ? 'bg-gray-800 text-white border-l-4 border-primary-600' : '' }}">
    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<a href="{{ route('admin.chatbot.knowledge.index') }}" class="flex items-center pl-12 pr-6 py-2 text-gray-400 hover:bg-gray-800 hover:text-white transition text-sm {{ request()->routeIs('admin.chatbot.knowledge*') ? 'bg-gray-800 text-white' : '' }}">
