...
>
    ...
    >
        ...
        <div class="px-3 py-3 border-t border-gray-100 bg-white">
            ...
            {{-- Footer: asistent text + WhatsApp transfer --}}
            <div class="flex items-center justify-between mt-2">
                <p class="text-xs text-gray-400">Asistent AI · MeseriasiOnline.ro</p>
                <a
                    :href="whatsappUrl()"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="flex items-center gap-1 text-xs text-green-600 hover:text-green-700 font-medium transition"
                    title="Continuă pe WhatsApp cu un consultant"
                >
...
document.addEventListener('alpine:init', () => {
    Alpine.data('chatbotWidget', () => ({
        ...
        whatsappUrl() {
            const phone = '{{ config('services.whatsapp.number', '40740173581') }}';
            const lastUserMsg = [...this.messages].reverse().find(m => m.role === 'user');
            const text = lastUserMsg
                ? `Bună ziua! Am o întrebare: ${lastUserMsg.content}`
                : 'Bună ziua! Aș dori să vorbesc cu un consultant.';
